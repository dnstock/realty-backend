...
def get_all(db: Session, skip: int = 0, limit: int = 10) -> PaginatedResults:
    return base.get_all(db=db, model=Unit, skip=skip, limit=limit)

def get_all_from_parent(db: Session, parent_id: int, skip: int = 0, limit: int = 10) -> PaginatedResults:
    return base.get_all_from_parent(db=db, model=Unit, parent_key='building_id', parent_value=parent_id, skip=skip, limit=limit)
