...
def validate_property_ownership(property_id: int, current_user: UserSchema.Read) -> None:
    validate_ownership(model_name='Property', resource_id=property_id, current_user=current_user)
...
@router.get('/', response_model=list[PropertySchema.Read])
def read_properties(
...
@router.get('/{property_id}/buildings/', response_model=list[BuildingSchema.Read])
def read_buildings(
